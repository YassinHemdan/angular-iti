...
export function generateQuestion(): Question {
    ...
    let operator: string = operators[getRandInt(0, 3)];
    let num1: number = getRandInt(1, 50);
    let num2: number = getRandInt(1, 50);
    let answer: number;
...
export function showSection(activeSection: HTMLDivElement, sections: HTMLDivElement[]): void {
    for (let i=0 ; i<sections.length ; i++) {
        sections[i].hidden = sections[i] !== activeSection;
    }
}

export function getFeedbackMessage(score: number, total: number): string {
